...
#define FILENAME "key_log_node2.txt"

int currentNode = N2;
int currentNode_packetType = DH_PACKET_TYPE_N2;
...
void SendDH_Keys(void *arg)
{
	...
	//
		if (skip_send == 0)
		{
			printf("Going from node2 to router\n");
			switch(i)
			{
				case 1:
					print_dhPacket(packet_send);
			...
			while(1)
			{
				...
				if(pckt->hdr.packet_type == DH_PACKET_TYPE_R1B 
				...
				{
					printf("Receiving from router to node2\n");
					print_dhPacket(pckt);
					switch(i)
					{
						case 1:
					...
					printf("sending flag from node2 to router\n");
					switch(i)
					{
						case 1:
							print_dhPacket(pckt);
...
void *ReceiveDH_Keys(void *inputargv)
{
	...
	while(1)
	{
		...
		{
			printf("Received from router to node2\n");
			printf("pckt value of packet_type is %d\n", pckt->hdr.packet_type);
			printf("pckt value of src address is %d\n", pckt->hdr.src_addr);
			printf("pckt value of dst address is %d\n", pckt->hdr.dest_addr);
			printf("pckt value of sequence number is %d\n", pckt->hdr.seq_no);
