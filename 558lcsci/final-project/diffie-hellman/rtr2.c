...
int link_R2R1_down = 0;
int link_R2R3_down = 0;
...
void *sniffDH_intf_func(void* arg)
{
	...
	while(1)
	{
		...
		if	(!strcmp(threadArg->sniffEthPort, "eth3") && pckt->hdr.packet_type == DH_PACKET_TYPE_N3)
		{
		...
		{
			if (!strcmp(threadArg->sniffEthPort, "eth2") && pckt->hdr.packet_type == DH_PACKET_TYPE_R1D)
			{
			...
			else
			{
				...
				{
					if (!strcmp(threadArg->sniffEthPort, "eth4") && pckt->hdr.packet_type == DH_PACKET_TYPE_R3D)
					{
...
//	unsigned char*new_packet_buffer=(unsigned char*)malloc(sizeof(unsigned char)*(frame_len-HDR_SIZE));
	...
	while(1)
	{
		...
		/*******************link failure******************/
		if (!strcmp(threadArg->sniffEthPort, "eth4") && (pckt->packet_type == FT_PACKET_TYPE_R2A))
		{
			printf("Sniffed on eth4\n");
			...
			send_file_data(sock_sniff1, &device, buffer+HDR_SIZE  , "eth3");
			
		}

		/*******************link failure******************/
		else if (!strcmp(threadArg->sniffEthPort, "eth2") && (pckt->packet_type == FT_PACKET_TYPE_R2A))
		{
			printf("Sniffed on eth2\n");
			printf("pckt seq no = %d\n", pckt->seq_no);
		...
		if (!strcmp(threadArg->sniffEthPort, "eth4") && (pckt->packet_type == FT_PACKET_TYPE_R2B))
		{
		}
		else if(!strcmp(threadArg->sniffEthPort, "eth3") && (pckt->packet_type == ACK_PACKET_TYPE_N3))
		{	 
									
			pckt->packet_type = ACK_PACKET_TYPE_R2A;
			//printf("\nACK packet\n");
			//strcpy(send_interf, route[pckt->dest_addr]->sendInterface); //FIX dest address in the node codes
			send_file_data(sock_sniff1, &device, buffer, "eth2");
			//usleep(1);
...
void *sniff_linkstatus_func(void* arg)
{
	...
	while(1)
	{
		...
		if	(pckt->packet_type == LINKSTATE_PACKET_TYPE && pckt->seq_no == LS_REQUEST
			&& (pckt->padding2 == DH_PACKET_TYPE_R1D) && !strcmp(threadArg->sniffEthPort, "eth2"))
		{
			printf("sniffed on %s\n", threadArg->sniffEthPort);
			printf("pckt value of packet_type is %d\n", pckt->packet_type);
			...
			pckt->padding2 = DH_PACKET_TYPE_R2A;
			send_file_data(sock_sniff, &device, buffer, "eth2");
		}
		else
		{
			if (pckt->packet_type == LINKSTATE_PACKET_TYPE && pckt->seq_no == LS_REQUEST
				&& (pckt->padding2 == DH_PACKET_TYPE_R3D) && !strcmp(threadArg->sniffEthPort, "eth4"))
			{
				printf("sniffed on %s\n", threadArg->sniffEthPort);
				printf("pckt value of packet_type is %d\n", pckt->packet_type);
				printf("pckt value of seq no %d\n", pckt->seq_no);
				...
				pckt->src_addr = R2;
				pckt->seq_no = LS_REPLY;
				pckt->padding2 = DH_PACKET_TYPE_R2B;
...
void *send_linkstatus_func(void* arg)
{
	...
	while(1)
	{
		...
		printf("pckt value of dest is %d\n", pckt->dest_addr);
		printf("pckt value of seq no %d\n", pckt->seq_no);
		printf("Link status packet sent from %s.\n", threadArg->sniffEthPort);
		for (i = 0; i < 3; ++i)
		{
			send_file_data(sock_sniff, &device, buffer, "eth2");
		}
		while(1)
		{
			...
			if	(pckt->packet_type == LINKSTATE_PACKET_TYPE && pckt->seq_no == LS_REPLY 
				&& pckt->src_addr == temp_dest_addr && pckt->padding2 == DH_PACKET_TYPE_R1D)
			{
				printf("sniffed on %s\n", threadArg->sniffEthPort);
				printf("LSpckt value of packet_type is %d\n", pckt->packet_type);
				printf("LSpckt value of seq no %d\n", pckt->seq_no);
				printf("R2 === R1 link active.\n");
				strcpy(route[R1]->sendInterface, "eth2");
				route[R1]->DHpacketType_send = DH_PACKET_TYPE_R2A;
				route[R1]->FTpacketType_send = FT_PACKET_TYPE_R2A;
				break;
			}
			else
			{
				if (end_count >= 1000)
				{
					printf("End count reached for R1\n");
					if (link_R2R3_down == 0)
					{
						strcpy(route[R1]->sendInterface, "eth4");
						route[R1]->DHpacketType_send = DH_PACKET_TYPE_R2B;
						route[R1]->FTpacketType_send = FT_PACKET_TYPE_R2B;
						link_R2R1_down = 1;
						printf("R2 === R1 link is down.\n");
					}
					else
					{
		...
		while(1)
		{
			...
			data_size= recvfrom(sock_sniff, buffer, frame_len, 0, &saddr, (socklen_t*)&saddr_size);
			if	(pckt->packet_type == LINKSTATE_PACKET_TYPE && pckt->seq_no == LS_REPLY 
				&& pckt->src_addr == temp_dest_addr && pckt->padding2 == DH_PACKET_TYPE_R3D)
			{
				printf("sniffed on %s\n", threadArg->sniffEthPort);
				printf("pckt value of packet_type is %d\n", pckt->packet_type);
				printf("pckt value of seq no %d\n", pckt->seq_no);
				printf("R2 === R3 link active.\n");
				strcpy(route[R3]->sendInterface, "eth4");
				route[R3]->DHpacketType_send = DH_PACKET_TYPE_R2B;
				route[R3]->FTpacketType_send = FT_PACKET_TYPE_R2B;
				break;
			}
			else
			{
