...
char *sniffPort[4];
int link_R1R2_down = 0;
int link_R1R3_down = 0;
...
void setupRoutingTable()
{
	...
	route[1]->DHpacketType_send = DH_PACKET_TYPE_R1A;
	route[1]->FTpacketType_send = FT_PACKET_TYPE_R1A;

	route[2]->destNode = 2;
	strcpy(route[2]->sendInterface, "eth1");
	route[2]->DHpacketType_send = DH_PACKET_TYPE_R1B;
	route[2]->FTpacketType_send = FT_PACKET_TYPE_R1B;

	route[3]->destNode = 3;
	strcpy(route[3]->sendInterface, "eth2");
	route[3]->DHpacketType_send = DH_PACKET_TYPE_R1D;
	route[3]->FTpacketType_send = FT_PACKET_TYPE_R1D;

	route[4]->destNode = 4;
	strcpy(route[4]->sendInterface, "eth2");
	route[4]->DHpacketType_send = DH_PACKET_TYPE_R1D;
	route[4]->FTpacketType_send = FT_PACKET_TYPE_R1D;

	route[5]->destNode = 5;
	strcpy(route[5]->sendInterface, "eth4");
	route[5]->DHpacketType_send = DH_PACKET_TYPE_R1C;
	route[5]->FTpacketType_send = FT_PACKET_TYPE_R1C;

	route[6]->destNode = 6;
	strcpy(route[6]->sendInterface, "eth4");
	route[6]->DHpacketType_send = DH_PACKET_TYPE_R1C;
	route[6]->FTpacketType_send = FT_PACKET_TYPE_R1C;

	route[7]->destNode = 7;
	strcpy(route[7]->sendInterface, "eth3");
	route[7]->DHpacketType_send = DH_PACKET_TYPE_R1A;
	route[7]->FTpacketType_send = FT_PACKET_TYPE_R1A;

	route[8]->destNode = 8;
	strcpy(route[8]->sendInterface, "eth2");
	route[8]->DHpacketType_send = DH_PACKET_TYPE_R1D;
	route[8]->FTpacketType_send = FT_PACKET_TYPE_R1D;

	route[9]->destNode = 9;
	strcpy(route[9]->sendInterface, "eth4");
	route[9]->DHpacketType_send = DH_PACKET_TYPE_R1C;
	route[9]->FTpacketType_send = FT_PACKET_TYPE_R1C;
}
...
void *sniffDH_intf_func(void* arg)
{
	...
	while(1)
	{
		...
		if	(!strcmp(threadArg->sniffEthPort, "eth3") && pckt->hdr.packet_type == DH_PACKET_TYPE_N1 )
		{
		...
		{
			if (!strcmp(threadArg->sniffEthPort, "eth1") && pckt->hdr.packet_type == DH_PACKET_TYPE_N2 )
			{
			...
			{
				if (!strcmp(threadArg->sniffEthPort, "eth2") && (pckt->hdr.packet_type == DH_PACKET_TYPE_R2A))
				{
				...
				{
					if (!strcmp(threadArg->sniffEthPort, "eth4") && (pckt->hdr.packet_type == DH_PACKET_TYPE_R3A))
					{
...
//	unsigned char*new_packet_buffer=(unsigned char*)malloc(sizeof(unsigned char)*(frame_len-HDR_SIZE));
	...
	while(1)
	{
		...
		if (!strcmp(threadArg->sniffEthPort, "eth1") && (pckt->packet_type == FT_PACKET_TYPE_R1B))
		{
			printf("Sniffed on eth1\n");
...
/*
    		tmp_hdr_decrypted=buffer+HDR_SIZE+HDR_SIZE+HDR_SIZE;

	    	EVP_DecryptInit(&ctx_enc,EVP_aes_128_cbc(),key,iv);
    		EVP_DecryptUpdate(&ctx_enc,new_packet_buffer,&outlen1,tmp_hdr_decrypted,HDR_SIZE);
...
*/
    		...
    		{
    			send_file_data(sock_sniff1, &device, buffer+HDR_SIZE , "eth4");
    		}
			// send_file_data(sock_sniff1, &device, buffer+HDR_SIZE , "eth2");
			c++;
		
		}
		else if(!strcmp(threadArg->sniffEthPort, "eth3") && (pckt->packet_type == ACK_PACKET_TYPE_N2))
		{	 									
		}
		if (!strcmp(threadArg->sniffEthPort, "eth2") && (pckt->packet_type == FT_PACKET_TYPE_R3B))
		{
		}
		else if(!strcmp(threadArg->sniffEthPort, "eth2") && (pckt->packet_type == ACK_PACKET_TYPE_R2A))
		{	 
									
			pckt->packet_type = ACK_PACKET_TYPE_R1B;
			//strcpy(send_interf, route[pckt->dest_addr]->sendInterface); //FIX dest address in the node codes
			send_file_data(sock_sniff1, &device, buffer, "eth1");
			//usleep(1);
...
void *sniff_broadcast_intf_func(void* arg)
{
	...
	while(1)
	{
		memset(buffer, 0, frame_len);
		data_size= recvfrom(sock_sniff, buffer, frame_len, 0, &saddr, (socklen_t*)&saddr_size);
		if	(pckt->packet_type == BROADCAST_PACKET_TYPE )
			// && pckt->padding1 > 0)
		{
			printf("sniffed on %s\n", threadArg->sniffEthPort);
			printf("pckt value of packet_type is %d\n", pckt->packet_type);
...
void *sniff_linkstatus_func(void* arg)
{
	...
	while(1)
	{
		...
		if	(pckt->packet_type == LINKSTATE_PACKET_TYPE && pckt->seq_no == LS_REQUEST
			&& (pckt->padding2 == DH_PACKET_TYPE_R2A) && !strcmp(threadArg->sniffEthPort, "eth2"))
		{
			printf("sniffed on %s\n", threadArg->sniffEthPort);
			printf("pckt value of packet_type is %d\n", pckt->packet_type);
			...
			pckt->padding2 = DH_PACKET_TYPE_R1D;
			send_file_data(sock_sniff, &device, buffer, "eth2");
		}
		else
		{
			if (pckt->packet_type == LINKSTATE_PACKET_TYPE && pckt->seq_no == LS_REQUEST
				&& (pckt->padding2 == DH_PACKET_TYPE_R3A) && !strcmp(threadArg->sniffEthPort, "eth4"))
			{
				printf("sniffed on %s\n", threadArg->sniffEthPort);
				printf("pckt value of packet_type is %d\n", pckt->packet_type);
				printf("pckt value of seq no %d\n", pckt->seq_no);
				...
				pckt->src_addr = R1;
				pckt->seq_no = LS_REPLY;
				pckt->padding2 = DH_PACKET_TYPE_R1C;
	...
	while(1)
	{
		...
		while(1)
		{
			...
			if	(pckt->packet_type == LINKSTATE_PACKET_TYPE && pckt->seq_no == LS_REPLY 
				&& pckt->src_addr == temp_dest_addr && pckt->padding2 == DH_PACKET_TYPE_R2A)
			{
				printf("sniffed on %s\n", threadArg->sniffEthPort);
				printf("LSpckt value of packet_type is %d\n", pckt->packet_type);
				printf("LSpckt value of seq no %d\n", pckt->seq_no);
				printf("R1 === R2 link active.\n");
				strcpy(route[R2]->sendInterface, "eth2");
				route[R2]->DHpacketType_send = DH_PACKET_TYPE_R1D;
				route[R2]->FTpacketType_send = FT_PACKET_TYPE_R1D;
				break;
			}
			else
			{
				if (end_count >= 1000)
				{
					printf("End count reached for R2\n");
					if (link_R1R3_down == 0)
					{
						strcpy(route[R2]->sendInterface, "eth4");
						route[R2]->DHpacketType_send = DH_PACKET_TYPE_R1C;
						route[R2]->FTpacketType_send = FT_PACKET_TYPE_R1C;
						link_R1R2_down = 1;
						printf("R1 === R2 link is down.\n");
					}
					else
					{
		...
		while(1)
		{
			...
			data_size= recvfrom(sock_sniff, buffer, frame_len, 0, &saddr, (socklen_t*)&saddr_size);
			if	(pckt->packet_type == LINKSTATE_PACKET_TYPE && pckt->seq_no == LS_REPLY 
				&& pckt->src_addr == temp_dest_addr && pckt->padding2 == DH_PACKET_TYPE_R3A)
			{
				printf("sniffed on %s\n", threadArg->sniffEthPort);
				printf("pckt value of packet_type is %d\n", pckt->packet_type);
				printf("pckt value of seq no %d\n", pckt->seq_no);
				printf("R1 === R3 link active.\n");
				strcpy(route[R3]->sendInterface, "eth4");
				route[R3]->DHpacketType_send = DH_PACKET_TYPE_R1C;
				route[R3]->FTpacketType_send = FT_PACKET_TYPE_R1C;
				break;
			}
			else
			{
				if (end_count >= 1000)
				{
					if (link_R1R2_down == 0)
					{
						strcpy(route[R3]->sendInterface, "eth2");
						route[R3]->DHpacketType_send = DH_PACKET_TYPE_R1D;
						route[R3]->FTpacketType_send = FT_PACKET_TYPE_R1D;
						link_R1R3_down = 1;
						printf("R1 === R3 link is down.\n");
					}
					else
					{
						printf("Both the links are down.\n");
