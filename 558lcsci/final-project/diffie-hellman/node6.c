...
#define ETHERNET_PORT_NODE "eth3"

#define FILENAME "key_log_node6.txt"

int currentNode = N6;
int currentNode_packetType = DH_PACKET_TYPE_N6;
...
void SendDH_Keys(void *arg)
{
	...
	//
		if (skip_send == 0)
		{
			printf("Going from node6 to router\n");
			switch(i)
			{
				case 1:
					print_dhPacket(packet_send);
			...
			while(1)
			{
				...
				if(pckt->hdr.packet_type == DH_PACKET_TYPE_R3C 
				...
				{
					printf("Receiving from router to node6\n");
					print_dhPacket(pckt);
					switch(i)
					{
						case 1:
					...
					printf("sending flag from node6 to router\n");
					switch(i)
					{
						case 1:
							print_dhPacket(pckt);
...
void *ReceiveDH_Keys(void *inputargv)
{
	...
	while(1)
	{
		...
		{
			printf("Received from router to node6\n");
			printf("pckt value of packet_type is %d\n", pckt->hdr.packet_type);
			printf("pckt value of src address is %d\n", pckt->hdr.src_addr);
			printf("pckt value of dst address is %d\n", pckt->hdr.dest_addr);
			printf("pckt value of sequence number is %d\n", pckt->hdr.seq_no);
