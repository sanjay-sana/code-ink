...
void setupRoutingTable()
{
	...
	route[1]->destNode = 1;
	strcpy(route[1]->sendInterface, "eth4");
	...
	route[9]->destNode = 9;
	strcpy(route[9]->sendInterface, "eth1");
...
void *sniffDH_intf_func(void* arg)
{
	...
	while(1)
	{
		...
		if	(!strcmp(threadArg->sniffEthPort, "eth4") && pckt->hdr.packet_type == DH_PACKET_TYPE_N1 )
		{
		...
		{
			if (!strcmp(threadArg->sniffEthPort, "eth3") && pckt->hdr.packet_type == DH_PACKET_TYPE_N2 )
			{
			...
			else
			{
				...
				{
					if (!strcmp(threadArg->sniffEthPort, "eth1") && (pckt->hdr.packet_type == DH_PACKET_TYPE_R3A))
					{
...
//	unsigned char*new_packet_buffer=(unsigned char*)malloc(sizeof(unsigned char)*(frame_len-HDR_SIZE));
	...
	while(1)
	{
		...
		if (!strcmp(threadArg->sniffEthPort, "eth3") && (pckt->packet_type == FT_PACKET_TYPE_R1B))
		{
		...
		{	 
			...
			send_file_data(sock_sniff1, &device, buffer, "eth3");
			//usleep(1);
