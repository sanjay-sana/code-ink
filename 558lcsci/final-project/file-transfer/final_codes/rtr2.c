...
//	unsigned char*new_packet_buffer=(unsigned char*)malloc(sizeof(unsigned char)*(frame_len-HDR_SIZE));
	...
	while(1)
	{
		...
		if (!strcmp(threadArg->sniffEthPort, "eth3") && (pckt->packet_type == FT_PACKET_TYPE_R2A))
		{

			printf("writitng to eth4" );
    		EVP_CIPHER_CTX ctx_enc,ctx_dec;
    		int outlen1=HDR_SIZE;
    		unsigned char key[16] = {0};
    		unsigned char iv[16] = {0};
		...
		else if(!strcmp(threadArg->sniffEthPort, "eth4") && (pckt->packet_type == ACK_PACKET_TYPE_N3))
		{	 
		...
		/*******************link failure******************/
		if (!strcmp(threadArg->sniffEthPort, "eth2") && (pckt->packet_type == FT_PACKET_TYPE_R2A))
		{
    		EVP_CIPHER_CTX ctx_enc,ctx_dec;
    		int outlen1=HDR_SIZE;
    		unsigned char key[16] = {0};
