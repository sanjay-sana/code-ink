...
#define FILENAME "jitteroutput.txt"

int curr_packet_no=0;
char *file_array=NULL;
int file_flag=0;
int file_length=INP_FILE_LEN;
int seq_no_max;
int ending_flag=0;
int disp=0;
FILE *fd1;
...
void process_packet_intf3(u_char *args, const struct pcap_pkthdr *header,const u_char *old_buffer)
{
	...
	{
		inter_arrival_time = (delay_start.tv_sec + (double)(delay_start.tv_usec/1e6)) - temp_start;
	}
	printf("Inter arrival time: %f\n", inter_arrival_time);
	temp_start = delay_start.tv_sec + (double)(delay_start.tv_usec/1e6);
