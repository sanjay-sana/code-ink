...
#define FILENAME_fromA "testfile_lab5_copy_AB.bin"
#define FILENAME_fromC "testfile_lab5_copy_CB.bin"
...

{
	...
	while(curr_packet_no<seq_no_max)
	{
		...
		unsigned char src_mac[6] = {0xA0, 0x36, 0x9F, 0x08, 0x53, 0xAE};
...
void* recv_data_func(void* arg)
{
	...
	if(dir_recv==0)
		printf("The inbound direction for incoming packets at nodeB is successfully set\n");
	if(dir_recv<0)
	{
...
int main(int argc, char *argv[])
{
	switch(argc)
	{
		...
		// 	{
		// 		mycase=111;	
		// 		printf("The node B is now running for receiving the packets from other nodes\n");
		// 	}
		// 	else
		// 	{
		...
		case 3 :
			if(strcmp(argv[1],"do_send")==0)
			{
				...
				{
					printf("The nodeB will now be sending packets to nodeA\n");
					mycase=222;
					nodea_flag=1;
				}
				else
				{
					...
					{
						printf("The nodeB will now be sending packets to nodeC\n");
						mycase=333;
						nodec_flag=1;
					}
					else
		...
		case 4 :
			...
			{
				if (!strcmp(argv[2],"nodeA") || !strcmp(argv[2],"nodeC"))
				{
					if (!strcmp(argv[3],"nodeC") || !strcmp(argv[3],"nodeA"))
					{
						printf("Gotcha...\n");
						mycase =  444;
					}
