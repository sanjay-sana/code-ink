...
#define FILENAME_fromB "testfile_lab5_copy_BA.bin"
#define FILENAME_fromC "testfile_lab5_copy_CA.bin"
...
int curr_packet_no_fromC=0;
char *file_array_fromB=NULL;
char *file_array_fromC=NULL;
...

{
	...
	while(curr_packet_no<seq_no_max)
	{
		...
		unsigned char src_mac[6] = {0xA0, 0x36, 0x9F, 0x0A, 0x5B, 0x76};
		...
		int bytes_written=pcap_inject(pcap_send, buffer, NEW_ETH_FRAME_LEN);
		if (bytes_written==-1)
		{
...
void process_packet_intf0(u_char *args, const struct pcap_pkthdr *header,const u_char *old_buffer)
{
	...
	if(pckt->hdr.packet_type==9999)
	{
		if(pckt->hdr.src_addr==1)
		{
			...
			printf("The icoming packet is coming from node B\n");
		}
		if(pckt->hdr.src_addr==2)
		{
			total_pckt_recv_fromC++;
			printf("The incoming packet is coming from node C\n");
		}
		...
		if(pckt->hdr.src_addr==2)
			curr_packet_no_fromC=pckt->hdr.seq_no;
		if(pckt->hdr.seq_no==0 && pckt->hdr.src_addr==1 )
			file_array_fromB=(char*)malloc(file_length_recv*sizeof(char));
		if(pckt->hdr.seq_no==0 && pckt->hdr.src_addr==2 )
			file_array_fromC=(char*)malloc(file_length_recv*sizeof(char));
		...
		if(pckt->hdr.src_addr==1)
		{
			...
			if(total_pckt_recv_fromB == TOT_PKT)
			{
				...
				fwrite(file_array_fromB, INP_FILE_LEN, 1,fd1);
				struct timeval end_fromB;
				gettimeofday(&end_fromB, NULL);
				printf("The file writing for packets coming from B is completed at the following timestamp : %f\n", end_fromB.tv_sec + (double)(end_fromB.tv_usec/1e6));
				fflush(fd1);
				fclose(fd1);
		...
		if(pckt->hdr.src_addr==2)
		{
			if (file_length_recv - (curr_packet_no_fromC * DATA_INSIDE_LENGTH) > DATA_INSIDE_LENGTH)
			{
				memcpy( &file_array_fromC[curr_packet_no_fromC * DATA_INSIDE_LENGTH] ,pckt->data_inside, DATA_INSIDE_LENGTH);
			}
			else
			{
				memcpy( &file_array_fromC[curr_packet_no_fromC * DATA_INSIDE_LENGTH] ,pckt->data_inside, file_length_recv - (curr_packet_no_fromC * DATA_INSIDE_LENGTH));
			}
			printf("Value of total_pckt_recv from C is %d\n",total_pckt_recv_fromC);
			if(total_pckt_recv_fromC == TOT_PKT)
			{
				//printf("It did enter here\n");
				printf("All the packets received from C ... writing to file\n");
				fwrite(file_array_fromC, INP_FILE_LEN, 1,fd2);
				struct timeval end_fromC;
				gettimeofday(&end_fromC, NULL);
				printf("The file writing for packets coming from C is completed at the following timestamp : %f\n", end_fromC.tv_sec + (double)(end_fromC.tv_usec/1e6));
				fflush(fd2);
				fclose(fd2);
				printf("Writing of file done\n");
				// exit(0);
...
{
	fd1=fopen(FILENAME_fromB,"wb");
	fd2=fopen(FILENAME_fromC,"wb");

	char *file_name_fromB=(char*)malloc(30*sizeof(char));
	strcpy(file_name_fromB,FILENAME_fromB);

	char *file_name_fromC=(char*)malloc(30*sizeof(char));
	strcpy(file_name_fromC,FILENAME_fromC);
...
int main(int argc, char *argv[])
{
	switch(argc)
	{
		...
		// 	{
		// 		mycase=111;	
		// 		printf("The node A is now running for receiving the packets from other nodes\n");
		// 	}
		// 	else
		// 	{
		...
		case 3 :
			if(strcmp(argv[1],"do_send")==0)
			{
				...
				{
					printf("The nodeA will now be sending packets to nodeB\n");
					mycase=222;
					nodeb_flag=1;
				}
				else
				{
					if(strcmp(argv[2],"nodeC")==0)
					{
						printf("The nodeA will now be sending packets to nodeC\n");
						mycase=333;
						nodec_flag=1;
					}
					else
		...
		case 4 :
			...
			{
				if (!strcmp(argv[2],"nodeB") || !strcmp(argv[2],"nodeC"))
				{
					if (!strcmp(argv[3],"nodeC") || !strcmp(argv[3],"nodeB"))
					{
						// printf("Gotcha...\n");
						mycase =  444;
					}
