...
void send_data_func()
{
	...
	while(curr_packet_no<seq_no_max)
	{
		...
		unsigned char dest_mac[6] = {0xA0, 0x36, 0x9F, 0x08, 0x53, 0xAE};
		...
		rand_delay = randomDelay(pckt->hdr.seq_no);
		sleep(rand_delay);
