...
void print_icmp_packet(const u_char * Buffer , int Size)
{
	...
	if (strcmp(my_mac, dest_mac) == 0)
	{
		...
		if (ip_found == 1)
		{
			...
			printf("Packet dest mac address: %.2X:%.2X:%.2X:%.2X:%.2X:%.2X \n", eth->h_dest[0] , eth->h_dest[1] , eth->h_dest[2] , eth->h_dest[3] , eth->h_dest[4] , eth->h_dest[5]);
			char *mac_byte = (char *)malloc(2*sizeof(char));
			memcpy(mac_byte, entry_no[j].next_hop_mac, 2);
			eth->h_dest[0] = stringToHex(mac_byte);

			memcpy(mac_byte, entry_no[j].next_hop_mac+3, 2);
			eth->h_dest[1] = stringToHex(mac_byte);

			memcpy(mac_byte, entry_no[j].next_hop_mac+6, 2);
			eth->h_dest[2] = stringToHex(mac_byte);

			memcpy(mac_byte, entry_no[j].next_hop_mac+9, 2);
			eth->h_dest[3] = stringToHex(mac_byte);

			memcpy(mac_byte, entry_no[j].next_hop_mac+12, 2);
			eth->h_dest[4] = stringToHex(mac_byte);
		
			memcpy(mac_byte, entry_no[j].next_hop_mac+15, 2);
			eth->h_dest[5] = stringToHex(mac_byte);

			printf("Mod dest mac address: %.2X:%.2X:%.2X:%.2X:%.2X:%.2X \n", eth->h_dest[0] , eth->h_dest[1] , eth->h_dest[2] , eth->h_dest[3] , eth->h_dest[4] , eth->h_dest[5]);

			// printf("Interface: %s\n", entry_no[j].interface);
			memset(my_mac, 0, sizeof(my_mac));
			memset(my_recv_ip, 0, sizeof(my_recv_ip));
			getMyMacAddress(entry_no[j].interface, my_mac, my_recv_ip);
			printf("Packet src mac address: %.2X:%.2X:%.2X:%.2X:%.2X:%.2X \n", eth->h_source[0] , eth->h_source[1] , eth->h_source[2] , eth->h_source[3] , eth->h_source[4] , eth->h_source[5]);
			memcpy(mac_byte, my_mac, 2);
			printf("%s\n", mac_byte);
			eth->h_source[0] = stringToHex(mac_byte);

			memcpy(mac_byte, my_mac+3, 2);
			printf("%s\n", mac_byte);
			eth->h_source[1] = stringToHex(mac_byte);

			memcpy(mac_byte, my_mac+6, 2);
			printf("%s\n", mac_byte);
			eth->h_source[2] = stringToHex(mac_byte);

			memcpy(mac_byte, my_mac+9, 2);
			eth->h_source[3] = stringToHex(mac_byte);

			memcpy(mac_byte, my_mac+12, 2);
			printf("%s\n", mac_byte);
			eth->h_source[4] = stringToHex(mac_byte);
		
			memcpy(mac_byte, my_mac+15, 2);
			printf("%s\n", mac_byte);
			eth->h_source[5] = stringToHex(mac_byte);
        ...
        sin.sin_addr.s_addr = inet_addr (entry_no[j].next_hop_ip);
